...
class Posvegamenu extends Module {
    ...
    public function __construct() {
        ...
        $this->description = $this->l('Vertical Mega Menu');
	  ...
	  //  $html[] = '<span><img src="'.$thumb.'" alt="" />' . $name . '</span>';
        ...
        if ($drawPopup)
        {
            ...
            if (count($activeChildren))
            {
                ...
                {
                    $html[] = '<div class="column blockright last blockHtmlRight"  style="float:left;">';
                    $html[] = $blockHtml;
                    $html[] = '</div>';
                }
...
//        $html[] = '</a>';
    ...
    public function hookDisplayHeader() {
		Media::addJsDef(
            array(
                'VMEGAMENU_POPUP_EFFECT' => Configuration::get($this->name . '_effect'),
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'SUP':
					...
					if (!is_null($supplier->id))
					{
						...
						$this->_menuLink .= '<div class ="pt_menu_sub pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'"><span>'.$supplier->name.'</span></a></div></div>'.PHP_EOL;
				...
				case 'SHOP':
					...
					{
						$link = new Link;
						$this->_menuLink .= '<div class ="pt_menu_sub pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'"><span>'.$supplier->name.'</span></a></div></div>'.PHP_EOL;
					}
					break;
				case 'LNK':
					...
					if (count($link))
					{
						...
						$this->_menuLink .= '<div class ="pt_menu pt_ver_menu_link"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</a></span></div></div>'.PHP_EOL;
					}
					break;
	...
	public function hookdisplayVegamenu($params)
	{
	...
	public function hookdisplayLeftColumn($params) {
        return $this->hookdisplayVegamenu($params);
    }
